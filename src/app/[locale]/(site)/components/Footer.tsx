...
const Footer: React.FC<FooterProps> = ({ params }) => {
  const { locale } = params;
  return (
    <div className="bg-title min-h-[416px] relative xl:top-[30px] overflow-x-clip">
      <div className="container  px-[16px] sm:px-[32px] md:px-[64px] lg:px-[112px] py-[30px] mx-auto">
        ...
        <div className="grid xl:grid-cols-4 lg:grid-cols-3 md:grid-cols-2 grid-cols-1 gap-10 mt-5">
          <div>
          ...
          <div className="relative z-30">
            ...
            <form>
              <input
                type="text"
                className="w-[277px] h-[44px] bg-transparent mb-3  text-[#ffffff] font-bold text-[13px] p-4 block rounded-[15px] border border-[#ffffff] outline-0"
                placeholder={locale == "en" ? "Full Name" : "الاسم بالكامل"}
                value={locale == "en" ? "Full Name" : "الاسم بالكامل"}
              />
              <input
                type="text"
                className="w-[277px] h-[44px] bg-transparent text-[#ffffff] mb-3 font-bold text-[13px] p-4 block rounded-[15px] border border-[#ffffff] outline-0"
                placeholder={locale == "en" ? "Phone Number" : "رقم الهاتف"}
                value={locale == "en" ? "Phone Number" : "رقم الهاتف"}
              />
              <button
                type="submit"
                className="w-[132px] h-[40px] hvr-pulse bg-[#7a4daa]  rounded-[15px] border border-[#ffffff] text-[#ffffff]"
              >
                {locale == "en" ? "Send Your Request Now" : "ارسل طلبك الان"}
              </button>
            </form>
          </div>
        </div>
